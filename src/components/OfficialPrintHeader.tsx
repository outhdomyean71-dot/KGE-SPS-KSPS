...
import { SchoolInfo, OfficePrintConfig } from '../types';
...
export const OfficialPrintHeader: React.FC<OfficialPrintHeaderProps> = ({
  ...
  const colorMode = printConfig?.colorMode || 'official';

  const schoolNameColorClass =
  ...
  return (
    <div className={`mb-6 space-y-3 border-b-2 ${borderClass} pb-4 ${className}`}>
      ...
      <div className="flex flex-row items-center justify-between text-xs font-semibold text-slate-900 leading-relaxed gap-4 w-full">
        ...
        <div className="text-left space-y-0.5 shrink-0">
          ...
          <p className={`font-bold ${schoolNameColorClass} leading-normal`}>
            {schoolName}
          </p>
        </div>
